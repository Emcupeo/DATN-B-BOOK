    ...
    <div class="w-full font-roboto">
      ...
      <div class="min-h-[calc(100vh-56px)] w-full bg-white p-10 rounded flex flex-col">
        ...
        <transition name="fade-slide" mode="out-in">
          <div :key="step">
            <!-- Step 1 -->
            <div v-if="step === 1" class="flex-1 flex flex-col items-center justify-center gap-12 min-h-[calc(100vh-350px)]">
            ...
            <!-- Step 2 -->
            <div v-else-if="step === 2" class="flex-1 flex flex-col items-center justify-center gap-8 min-h-[calc(100vh-350px)]">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-10 max-w-4xl w-full px-4">
                <div @click="chonApDung('congKhai')"
                  class="cursor-pointer bg-white border border-gray-300 rounded-2xl shadow-lg p-10 flex flex-col items-center text-center hover:scale-105 transition-all duration-300 hover:bg-gradient-to-tl hover:from-[#003366] hover:to-[#242124] group">
                  <svg class="w-16 h-16 mb-5 text-blue-600 group-hover:text-white" fill="currentColor" viewBox="0 0 20 20">
                    <path
              ...
              <div class="flex flex-col lg:flex-row gap-6 items-start">
                <form @submit.prevent="handleSubmit" class="flex-1 space-y-4">
                  <div class="grid grid-cols-2 gap-4 mb-4">
                    ...
                    <div class="col-span-2 sm:col-span-1" v-if="loaiGiamGia === 'phanTram'">
                      <label for="soPhanTramGiam" class="block mb-2 text-sm font-medium text-gray-900">Phần trăm giảm</label>
                    ...
                    <div class="col-span-2 sm:col-span-1">
                      <label for="giaTriDonHangToiThieu" class="block mb-2 text-sm font-medium text-gray-900">Giá trị đơn giảm tối thiểu</label>
                      <input v-model="newPhieu.giaTriDonHangToiThieu" type="number" name="giaTriDonHangToiThieu" id="giaTriDonHangToiThieu"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5"
                        placeholder="Giá trị đơn hàng tối thiểu" required />
                      <p v-if="errors.giaTriDonHangToiThieu" class="text-red-500 text-xs mt-1">{{ errors.giaTriDonHangToiThieu }}</p>
                    </div>
                    <div class="col-span-2 sm:col-span-1">
                ...
                <div v-if="showConfirmModal" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
                  <div class="bg-white rounded-lg p-6 max-w-md w-full mx-4">
                    <h3 class="text-lg font-semibold mb-4">Xác nhận {{ newPhieu.id ? "cập nhật" : "thêm" }} phiếu giảm giá</h3>
                    <p class="text-gray-600 mb-6">Bạn có chắc chắn muốn {{ newPhieu.id ? "cập nhật" : "thêm" }} phiếu giảm giá này không?</p>
                    <div class="flex justify-end gap-3">
                      <button @click="showConfirmModal = false" class="px-4 py-2 text-gray-600 hover:text-gray-800 font-medium">Hủy</button>
                      <button @click="confirmSubmit" class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 font-medium">Xác nhận</button>
                    </div>
                  </div>
                </div>
                ...
                <div v-if="loaiApDung === 'khachHang'" class="flex-1 pl-6">
                  ...
                  <div class="flex justify-center items-center mt-4 space-x-2">
                    ...
                    <span class="text-xs font-semibold text-gray-700">Trang {{ currentPage + 1 }} / {{ totalPages }}</span>
    ...
    async handleSubmit() {
      ...
      try {
        ...
        this.showNotification(
          this.isEdit ? 'Cập nhật phiếu giảm giá thành công!' : 'Thêm phiếu giảm giá thành công!',
          'success'
        );
        this.$router.push('/admin/phieu-giam-gia');
      } catch (error) {
