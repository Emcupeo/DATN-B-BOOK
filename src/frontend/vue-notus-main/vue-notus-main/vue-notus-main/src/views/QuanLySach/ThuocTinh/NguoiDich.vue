  ...
  <div class="min-h-screen w-full bg-gray-100 font-roboto">
    <div class="bg-white p-4 shadow rounded w-full">
      ...
      <div class="flex items-center justify-between mb-4">
        <h2 class="text-lg font-bold">Danh sách người dịch</h2>
      ...
      <div class="relative overflow-x-auto sm:rounded-lg w-full h-[calc(100vh-120px)]">
        <table class="w-full text-sm text-center text-gray-500">
          ...
          <tr>
            ...
            <th scope="col" class="px-6 py-3 cursor-pointer" @click="sortBy('maNguoiDich')">
              Mã người dịch
              <span v-if="sortKey === 'maNguoiDich'" class="ml-1">
            ...
            <th scope="col" class="px-6 py-3 cursor-pointer" @click="sortBy('tenNguoiDich')">
              Tên người dịch
              <span v-if="sortKey === 'tenNguoiDich'" class="ml-1">
                  <svg class="w-3 h-3 inline" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" :d="sortOrder === 1 ? 'M19 9l-7 7-7-7' : 'M5 15l7-7 7 7'"></path>
                  </svg>
                </span>
          ...
          <tbody>
          <tr class="bg-white border-b hover:bg-gray-50" v-for="(nguoiDich, index) in paginatedNguoiDich" :key="nguoiDich.id">
            <td class="px-6 py-4">{{ (currentPage - 1) * itemsPerPage + index + 1 }}</td>
            <td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">{{ nguoiDich.maNguoiDich }}</td>
            <td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">{{ nguoiDich.tenNguoiDich }}</td>
            <td class="px-6 py-4 flex justify-center space-x-3">
              <a href="#" @click="editNguoiDich(nguoiDich)" class="text-gray-600 hover:text-gray-800">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L6.832 19.82a4.5 4.5 0 01-1.897 1.13l-2.685.8.8-2.685a4.5 4.5 0 011.13-1.897L16.863 4.487zm0 0L19.5 7.125" />
                </svg>
              </a>
              <a href="#" @click="deleteNguoiDich(nguoiDich.id)" class="text-red-600 hover:text-red-800">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
          ...
          <tr class="bg-white border-b hover:bg-gray-50" v-if="paginatedNguoiDich.length == 0">
